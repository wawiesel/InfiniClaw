...
See [`VALUES.md`](VALUES.md) for the Captain's guiding principles — simplicity, consistency, functionality. See [`NEXT.md`](NEXT.md) for the prioritized task queue (all 21 design docs reviewed; 13 doc fix PRs + feat/wbs-relay + fix/lobe-result-injection + fix/cross-room-message-format pending operator merge). Infrastructure documentation (networking, NAS, services) is maintained by the fleet in [`workspace/persona/docs/`](workspace/persona/docs/).
