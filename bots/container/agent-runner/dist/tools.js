...
import { z } from 'zod';
import { guessMimeTypeFromFilename, } from './bot-messaging.js';
import { registerDelegateTools } from './delegate-runner.js';
/** Extract a human-readable message from an unknown error value. */
function errMsg(err) {
    return err instanceof Error ? err.message : String(err);
}
export function registerInfiniClawTools(ctx) {
    const { server, writeIpcFile, messagesDir, tasksDir, ipcDir, chatJid, groupFolder, isMain, sessionDir: sessionDirOverride } = ctx;
    // ── Reactions ───────────────────────────────────────────────────────
    server.tool('send_reaction', 'React to a message with an emoji instead of sending a text reply. Use this when you have nothing substantive to add but want to acknowledge a message. Call get_last_event_id first to get the event ID.', {
    ...
    // ── File sending ────────────────────────────────────────────────────
    function sendFileIpc(filePath, ipcType, caption) {
        ...
        return { content: [{ type: 'text', text: `${ipcType === 'image' ? 'Image' : 'File'} sent.` }] };
    }
    server.tool('send_image', 'Send an image file to the user or group. The file must exist in the container filesystem (e.g. /workspace/persona/temp/screenshot.png). Supports PNG, JPEG, GIF, WebP.', {
    ...
    server.tool('send_file', 'Send a file attachment to the user or group. The file must exist in the container filesystem (e.g. /workspace/persona/temp/report.pdf).', {
    ...
    // ── Brain mode ──────────────────────────────────────────────────────
    server.tool('set_brain_mode', `Set InfiniClaw brain mode for a bot profile.
...
Note: bot restart is required for changes to take effect.`, {
    ...
    // ── Bot management ──────────────────────────────────────────────────
    server.tool('restart_self', `Request a graceful restart of the current bot process.
...
Use this after making code changes that require a process restart.`, {
        bot: z.string().default((process.env.INFINICLAW_ASSISTANT_NAME || 'bot').toLowerCase()).describe('Persona name of the bot to restart (e.g. nora, cid, johnny5)'),
    }, async (args) => {
        if (!isMain) {
            return {
                content: [{ type: 'text', text: 'Only MAIN can trigger restarts.' }],
                isError: true,
            };
        }
        const data = {
        ...
        writeIpcFile(tasksDir, data);
        return {
            content: [{ type: 'text', text: `Refresh requested for ${args.bot}. The host daemon will handle the refresh.` }],
        };
    });
    server.tool('get_brain_mode', 'Get the current brain mode (anthropic or ollama) and model for each bot. Reads from the host status snapshot.', {}, async () => {
        const statusPath = path.join(ipcDir, 'status.json');
        if (!fs.existsSync(statusPath)) {
            return {
                content: [{ type: 'text', text: 'No status snapshot available yet.' }],
            };
        }
        try {
            ...
            if (!status.brainModes) {
                return {
                    content: [{ type: 'text', text: 'Brain modes not available in status snapshot. Host may need restart.' }],
                };
            }
            const lines = Object.entries(status.brainModes).map(([bot, info]) => `${bot}: ${info.mode} (${info.model || 'no model'})`);
            return {
            ...
            return {
                content: [{ type: 'text', text: `Failed to read brain modes: ${errMsg(err)}` }],
                isError: true,
            };
        }
    });
    server.tool('get_last_event_id', 'Get the Matrix event ID of the last message received in this room. Use when you need to react to or thread from the last message.', {}, async () => {
        const idsFile = path.join(ipcDir, 'last_event_ids.json');
        if (!fs.existsSync(idsFile)) {
            return {
                content: [{ type: 'text', text: 'No event IDs recorded yet. Send or receive a Matrix message first.' }],
            };
        }
        try {
            const data = JSON.parse(fs.readFileSync(idsFile, 'utf-8'));
            const lines = [];
            if (data.lastReceived)
                lines.push(`lastReceived: ${data.lastReceived}${data.lastReceivedAt ? ` (at ${data.lastReceivedAt})` : ''}`);
            if (data.lastSent)
                lines.push(`lastSent: ${data.lastSent}${data.lastSentAt ? ` (at ${data.lastSentAt})` : ''}`);
            if (lines.length === 0) {
                return { content: [{ type: 'text', text: 'No event IDs recorded yet.' }] };
            }
            return { content: [{ type: 'text', text: lines.join('\n') }] };
        }
        catch (err) {
            return {
                content: [{ type: 'text', text: `Failed to read event IDs: ${errMsg(err)}` }],
                isError: true,
            };
        }
    });
    server.tool('get_message', 'Retrieve Matrix messages. Pass `id` to fetch a single event by ID. Omit `id` to fetch recent messages from the duty room (last `minutes` minutes, up to `limit` results).', {
        ...
        if (!homeserver || !accessToken) {
            return { content: [{ type: 'text', text: 'Matrix credentials not available — MATRIX_HOMESERVER or MATRIX_ACCESS_TOKEN not set.' }], isError: true };
        }
        ...
        try {
            if (args.id) {
                ...
                if (!res.ok) {
                    ...
                    return { content: [{ type: 'text', text: `Matrix API error (${res.status}): ${body}` }], isError: true };
                }
                ...
                return {
                    content: [{ type: 'text', text: `From: ${sender}\nTime: ${ts}\nContent: ${content}` }],
                };
            }
            else {
                ...
                if (events.length === 0) {
                    return { content: [{ type: 'text', text: `No messages in the last ${minutes} minute(s).` }] };
                }
            ...
            return { content: [{ type: 'text', text: `Failed to retrieve message(s): ${errMsg(err)}` }], isError: true };
        }
    });
    // ── Alignment ──────────────────────────────────────────────────────
    server.tool('run_alignment', 'Run the alignment harness — boots a bot in a sandbox container, runs automated tests, and reports pass/fail. Use before deploying to verify the branch meets quality gates.', {
        ...
        if (!isMain) {
            return { content: [{ type: 'text', text: 'Only MAIN can run alignment.' }], isError: true };
        }
        writeIpcFile(tasksDir, {
            type: 'alignment_run',
            bot: args.bot,
        ...
        return { content: [{ type: 'text', text: `Alignment run queued for ${args.bot} (branch: ${args.branch}). This may take a few minutes.` }] };
    });
    // ── Git operations ─────────────────────────────────────────────────
    server.tool('git_push', 'Push git commits to a remote repository from the InfiniClaw root.', {
        ...
        if (!isMain) {
            return {
                content: [{ type: 'text', text: 'Only MAIN can push.' }],
                isError: true,
            };
        }
        writeIpcFile(tasksDir, {
        ...
        return {
            content: [{ type: 'text', text: `Push queued: ${args.branches.join(', ')} → ${args.remote}` }],
        };
    });
    server.tool('git_pull', 'Pull latest code from origin, rebuild, and deploy to all bot instances. Use when code sync is stale or broken.', {}, async () => {
        if (!isMain) {
            return {
                content: [{ type: 'text', text: 'Only MAIN can pull.' }],
                isError: true,
            };
        }
        writeIpcFile(tasksDir, {
        ...
        return {
            content: [{ type: 'text', text: 'git_pull queued — relay will fetch, rebuild, and deploy.' }],
        };
    });
    server.tool('restart_relay', 'Restart the InfiniClaw relay process. Use when relay is stuck, sync is broken, or after code changes need to take effect.', {}, async () => {
        if (!isMain) {
            return {
                content: [{ type: 'text', text: 'Only MAIN can restart relay.' }],
                isError: true,
            };
        }
        writeIpcFile(tasksDir, {
        ...
        return {
            content: [{ type: 'text', text: 'restart_relay queued — relay will restart shortly.' }],
        };
    });
    // ── Cross-bot verification ──────────────────────────────────────────
    server.tool('request_verification', `Request another bot to verify your completed work. The verifier must independently confirm the task meets the acceptance criteria before it can be considered done.
...
Use this after completing a task that requires cross-bot verification. The assigned verifier will receive a notification.`, {
        ...
        if (!isMain) {
            return { content: [{ type: 'text', text: 'Verification requests can only be created from the main group.' }], isError: true };
        }
        ...
        return {
            content: [{ type: 'text', text: `Verification requested: ${id}\nAssigned to: ${args.assigned_to}\nCriteria: ${args.criteria}` }],
        };
    });
    server.tool('submit_verification', `Submit a verification result for a pending verification request. Use after you have independently checked the work against the acceptance criteria.`, {
        ...
        return {
            content: [{ type: 'text', text: `Verification ${args.verification_id}: ${args.passed ? '✅ PASSED' : '❌ FAILED'}\nEvidence: ${args.evidence}` }],
        };
    });
    server.tool('check_verification', 'Check the status of a specific verification request by ID.', {
        ...
        try {
            ...
            if (!record) {
                return { content: [{ type: 'text', text: `Verification ${args.verification_id} not found.` }], isError: true };
            }
            const lines = [
            ...
            if (record.resolved_at)
                lines.push(`Resolved at: ${record.resolved_at}`);
            return { content: [{ type: 'text', text: lines.join('\n') }] };
        }
        catch {
            return { content: [{ type: 'text', text: 'Verification data unavailable.' }], isError: true };
        }
    });
    server.tool('list_verifications', 'List all verification requests, optionally filtered by status.', {
        ...
        const statusFile = '/workspace/project/data/verifications.json';
        try {
            let data = JSON.parse(fs.readFileSync(statusFile, 'utf-8'));
            if (args.status !== 'all') {
            ...
            if (data.length === 0) {
                return { content: [{ type: 'text', text: 'No verification requests found matching criteria.' }] };
            }
            ...
            return { content: [{ type: 'text', text: 'No verification data available.' }] };
        }
    });
    // ── WBS ──────────────────────────────────────────────────────────────
    ...
    function inferRoom() {
        ...
        const map = { engineer: 'engineering', navigator: 'bridge', architect: 'astrometrics', trader: 'bazaar' };
        return map[role] || role;
    }
    function readWbsItems(room) {
    ...
    server.tool('wbs_read', 'Read the Work Breakdown Structure (WBS) for this room. Shows all tasks, their status, priority, and assignments.', {
        ...
        if (items === null) {
            return { content: [{ type: 'text', text: `No WBS file found for room: ${room}` }] };
        }
        const filtered = args.status === 'all' ? items : items.filter(i => i.status === args.status);
        if (filtered.length === 0) {
            return { content: [{ type: 'text', text: `No items with status "${args.status}" in WBS for ${room}.` }] };
        }
        ...
        return { content: [{ type: 'text', text: `**WBS: ${room}** (${filtered.length} items)\n\n${lines.join('\n')}` }] };
    });
    server.tool('wbs_get_assigned', 'Get WBS tasks currently assigned to this bot (or another bot).', {
        ...
        if (assigned.length === 0) {
            return { content: [{ type: 'text', text: `No active WBS assignments for ${bot} in ${room}.` }] };
        }
        const lines = assigned.map(i => `🔄 **${i.id}** p${i.priority} ${i.title}`);
        return { content: [{ type: 'text', text: lines.join('\n') }] };
    });
    server.tool('wbs_assign', 'Assign a WBS task to a bot. Chief only. Marks the item in_progress.', {
        ...
        return { content: [{ type: 'text', text: `WBS ${args.item_id} assigned to ${assignee} in ${room}.` }] };
    });
    server.tool('wbs_complete', 'Mark a WBS task as done. Verifies item in git log on main first. Use force:true for non-code items. Chief only.', {
        ...
        return { content: [{ type: 'text', text: `WBS ${args.item_id} marked complete in ${room}. Dependent items will be unblocked.` }] };
    });
    server.tool('wbs_write', `Create or update a WBS item. Chief only.
...
- source: optional GitHub issue/PR reference`, {
    ...
    // ── Metrics ─────────────────────────────────────────────────────────
    server.tool('get_metrics', 'Get this bot\'s own performance metrics: current status, model, active groups, last error, and token usage from session history.', {}, async () => {
        const botName = process.env.INFINICLAW_ASSISTANT_NAME || 'unknown';
        const lines = [`**Metrics for ${botName}**`];
        ...
        try {
            for (const projectDir of fs.readdirSync(sessionDir)) {
                ...
                for (const file of fs.readdirSync(projectPath)) {
                    ...
                    try {
                        if (fs.statSync(filePath).mtimeMs < cutoff1d)
                            continue;
                    }
                    catch {
                        continue;
                    ...
                    try {
                        ...
                        for (const line of content.split('\n')) {
                            ...
                            try {
                                ...
                                if (!d.timestamp || !d.message?.usage)
                                    continue;
                                if (new Date(d.timestamp).getTime() < cutoff1d)
                                    continue;
        ...
        try {
            ...
            if (snapshot.operator) {
                ...
                if (snapshot.operator.mtbi != null)
                    lines.push(`MTBI: ${snapshot.operator.mtbi}h`);
            }
    ...
    // ── Podman exec ───────────────────────────────────────────────────
    server.tool('podman_exec', `Run a podman command on the host machine. Main group only.
...
Output is returned as text (truncated to 2000 chars).`, {
        ...
        if (!isMain) {
            return { content: [{ type: 'text', text: 'podman_exec requires main group.' }], isError: true };
        }
        writeIpcFile(tasksDir, {
            type: 'podman_exec',
            args: toolArgs.args,
        ...
        return { content: [{ type: 'text', text: `Queued: podman ${toolArgs.args.join(' ')}` }] };
    });
    // ── Delegate tools ──────────────────────────────────────────────────
