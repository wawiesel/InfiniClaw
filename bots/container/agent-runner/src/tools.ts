...
Use this after making code changes that require a process restart.`,
  ...
  server.tool(
    'run_alignment',
    'Run the alignment harness — boots a bot in a sandbox container, runs automated tests, and reports pass/fail. Use before deploying to verify the branch meets quality gates.',
    {
  ...
  server.tool(
    ...
    async (args) => {
      ...
      writeIpcFile(tasksDir, {
        ...
        assignee,
        room,
        bot: process.env.INFINICLAW_ASSISTANT_NAME || '',
        chatJid,
        groupFolder,
        timestamp: new Date().toISOString(),
      });
