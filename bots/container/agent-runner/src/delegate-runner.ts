...
export function registerDelegateTools(
  ...
  server.tool(
    'branch_to_thread',
    `Spawn a new Claude Branch Brain by outputting a {{branch}} signal. Output the returned text verbatim as your next message — do not add commentary or modify it. The relay intercepts the signal, posts the message as the thread root, and spawns the Branch Brain under it.`,
    {
    ...
    async (args) => {
      // Return the signal text. The bot outputs this verbatim as its next message.
      // The relay sees {{branch}}, strips it, posts the remaining text as the thread root,
      // and spawns the Branch Brain with that event ID. No IPC file needed.
      const signalText = `🌿 ${args.title} — ${args.objective}\n{{branch title="${args.title}" objective="${args.objective}"}}`;
      return {
        content: [{
          type: 'text' as const,
          text: signalText,
