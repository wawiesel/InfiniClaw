...
- **WBS MCP tools** (`tools.ts`): `wbs_read` (list items by room/status), `wbs_get_assigned` (tasks for a bot), `wbs_update` (mark status/assignee). Read `/workspace/data/wbs-{room}.json` (mounted from host `_runtime/data/`).
