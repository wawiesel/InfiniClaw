...
- **Lobe result delivery**: On lobe exit, writes `result-{lobeId}.json` to IPC input dir with `{type: "lobe_result", lobe_id, output, exitCode}`. `drainIpcInput()` in `index.ts` handles this type by formatting it as `[System: Lobe {id} completed]\n\n{output}` and injecting it into the bot's next turn.
