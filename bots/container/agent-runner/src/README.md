...
| `tools.ts` | MCP server tools: `get_last_event_id`, `get_message` (Matrix API via injected credentials), `send_reaction`, `send_image`, `send_file`, `set_brain_mode`, `get_brain_mode`, `get_metrics`, `restart_self`, `git_push`, `podman_exec`, `ts` | Implements `delegate_to_lobe` — spawns codex/gemini/claude/ollama sub-processes, threads results back. (`branch_to_thread` removed — use `{{branch}}` signal per 22-signals.md) |
...
<!-- auto-updated: 2026-03-31T14:45:23Z -->
