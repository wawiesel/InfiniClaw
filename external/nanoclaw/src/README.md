...
| `container-runner.ts` | Spawns podman containers for agent runs; syncs agent-runner source to per-group dirs when host files are newer; `ContainerOutput.isSessionError` signals stale session recovery |
