...
> **Status:** Relay lifecycle message broadcasting is not yet implemented. CO is currently elected from `fleet.json` at startup only. The bot runtime is ready to receive these messages once the relay sends them.
