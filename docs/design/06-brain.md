...
- **Bot-driven spawn** — the bot calls `branch_to_thread(title, objective)`. The tool posts the `🌿 title — objective` thread header automatically. The relay does not announce or spawn branches.
