...
- `01-operator.md` — Operator as escape hatch (max intelligence, idle in mature fleet, heavy use during development), bootstrap sequence (operator first → BehindTheCurtain → first ship → first bot), accounts, Captain communication routing (speaker default, 📞 broadcast), inter-operator inbox, x-commands, intervention as autonomy metric
