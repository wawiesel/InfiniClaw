...
- `03-ship.md` — Machine registry (`commissioned` flag vs bot `status`), relay process (loudspeaker tags, BehindTheCurtain mirror), speaker election, auto-sync loops, relay commands (fleet status, code pipeline, ship lifecycle), per-machine config
