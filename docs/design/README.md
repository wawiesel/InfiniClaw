...
- `03-ship.md` — Machine registry (`commissioned` flag vs bot `status`), relay process (loudspeaker tags, BehindTheCurtain mirror), speaker election, auto-sync loops, relay commands (!fleet, !push/!pull, !commission/!decommission, !operator), per-machine config
