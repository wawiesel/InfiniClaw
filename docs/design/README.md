...
- `08-threading.md` — Branch and Merge model, branch brains (interactive podman container, fallback to host), streaming output (assistant blocks posted as they arrive), concurrency limit, main-timeline summary (🧵 title — ✅ done after 30s debounce), context injection (fan-out to all active BB stdin), thread reactivation (follow-up in completed BB thread spawns new BB, 4h TTL in `branch-tasks.json`), lobes (MCP, any provider, quarters threads), correct branch protocol
