...
- `09-roles-and-rooms.md` — Roles, room topology, command hierarchy (Captain → Operator/Co → XO → Chiefs → Crew; Chief = highest-rank bot per room; Chief Navigator = XO), bot statuses (pip progression), lifecycle commands (!wake/!sleep/!report/!dismiss/!go — report/dismiss do lightweight restarts), quarters trigger rules, quarters as retrospective memory space
