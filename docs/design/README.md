...
- `05-bot.md` — Identity, bot attributes (triggerType, status, rank, activeBrainModel), mention/callout flow, response rules, unified display format (short: `<shipEmoji><locEmoji> <Name> <roleIcon><medal><healthEmoji><actEmoji>`, long adds bracketed values), health grades (A/B/C/F with thresholds, sleeping bots default A), special health states (emoji overrides only emoji, grade label always shown), activity levels (🔹/⚡/🔥 by tok/day, idle omitted, shown for all statuses including sleep), rank medals, location emojis, Matrix display name via `unifiedBotDisplay('short')`, boot progress (thread steps omit ship tag), branch brain spawn (container, host fallback), `!wake` restart behavior, resume behavior (IPC inject if active; else spawn), duty cycle status flow (onduty→retrospective→dream→ready, pip progression 🟢📝💤✅), bot metrics (response latency, branch brain success, crashes)
