...
**Bot usage (CO only):** Only the current Commanding Officer of a room is authorized to use the intercom. CO is dynamic — determined at deploy time by lowest rank among active bots on that room. If a machine goes down, the next-lowest-rank bot on the surviving machine becomes CO. When the CO calls `send_message` targeting a different room, the host relays through the target room's intercom account. Messages appear as `<BotName> (<SourceRoom>): <message>`.
