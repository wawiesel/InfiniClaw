...
**Startup:** Each bot reads `crew-status.json` (generated at deploy from fleet-wide presence) to set its initial badge.

**Runtime:** When the supervisor dismisses/joins a bot, it posts a lifecycle message to the room via intercom (e.g. `HERACLES: Cid stopped`). All bots in the room parse these messages to update their roster and re-evaluate CO. The supervisor also updates the local presence file so future deploys generate correct `crew-status.json`.
