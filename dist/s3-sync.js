...
async function downloadFile(client, bucket, key, filePath) {
...
export async function pushBot(root, bot) {
    const s3 = getClient();
    if (!s3)
        return;
...
export async function pullBot(root, bot) {
    const s3 = getClient();
    if (!s3)
        return;
    const instance = instanceDir(root, bot);
...
export async function pushAll(root) {
    const config = loadMachineConfig();
    if (!config.s3)
        return;
...
export async function pullAll(root) {
    const config = loadMachineConfig();
    if (!config.s3)
        return;
    for (const bot of config.bots) {
