...
export function grantMount(bot, rawPath, durationMinutes) {
...
export function revokeMount(bot, rawPath) {
    const expanded = expandTilde(rawPath);
    const list = loadAllowList();
    ...
    if (list.mounts[bot].length === before)
        return false;
    saveAllowList(list);
...
export function mountsForBot(bot) {
    const list = loadAllowList();
    const entries = list.mounts[bot] || [];
    const now = Date.now();
    const mounts = [];
    ...
    for (const entry of entries) {
        ...
        if (usedBasenames.has(basename)) {
            let n = 2;
            while (usedBasenames.has(`${basename}-${n}`))
                n++;
            basename = `${basename}-${n}`;
        }
