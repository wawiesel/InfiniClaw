...
function parseOutputMarkers(buffer) {
    const outputs = [];
...
export function runContainer(opts) {
    ...
    return new Promise((resolve) => {
        ...
        if (opts.firstOutputDeadlineMs && opts.firstOutputDeadlineMs > timeoutMs) {
            logger.warn({ firstOutputDeadlineMs: opts.firstOutputDeadlineMs, timeoutMs, group: opts.groupName }, 'firstOutputDeadlineMs exceeds container timeout — deadline will never fire');
        }
        const killOnTimeout = () => {
            timedOut = true;
            logger.error({ group: opts.groupName, containerName: opts.containerName }, 'Container timeout, stopping gracefully');
            exec(opts.stopCommand, { timeout: 15000 }, (err) => {
                if (err) {
                    logger.warn({ group: opts.groupName, containerName: opts.containerName, err }, 'Graceful stop failed, force killing');
        ...
        if (foDeadlineMs > 0) {
            firstOutputTimer = setTimeout(() => {
                if (!firstOutputReceived) {
                    logger.error({ group: opts.groupName, containerName: opts.containerName, deadlineMs: foDeadlineMs }, 'Container produced no output within first-output deadline, killing');
                    killOnTimeout();
                }
            }, foDeadlineMs);
        }
        ...
        container.stdout.on('data', (data) => {
            ...
            if (!firstOutputLogged) {
                ...
                logger.info({ group: opts.groupName, containerName: opts.containerName, ttfoMs }, 'Time to first output');
            }
            ...
            if (!stdoutTruncated) {
                ...
                if (chunk.length > remaining) {
                    ...
                    logger.warn({ group: opts.groupName, size: stdout.length }, 'Container stdout truncated due to size limit');
                }
                else {
                    stdout += chunk;
                }
        ...
        container.stderr.on('data', (data) => {
            ...
            for (const line of lines) {
                if (line)
                    logger.debug({ container: opts.groupFolder }, line);
            }
            if (stderrTruncated)
                return;
            ...
            if (chunk.length > remaining) {
                ...
                logger.warn({ group: opts.groupName, size: stderr.length }, 'Container stderr truncated due to size limit');
            }
            else {
                stderr += chunk;
            }
        });
        const settleOutputChain = (cb) => {
            const chainTimeoutMs = opts.outputChainTimeoutMs ?? 0;
            if (chainTimeoutMs > 0) {
                const chainTimer = setTimeout(() => {
                    logger.warn({ group: opts.groupName, containerName: opts.containerName }, 'outputChain stalled after container close, force-resolving');
                    cb();
                }, chainTimeoutMs);
                outputChain
                    .then(() => {
                    ...
                    .catch((err) => {
                    clearTimeout(chainTimer);
                    logger.error({ group: opts.groupName, err }, 'outputChain rejected after container close');
                    cb();
                });
            }
            else {
        ...
        container.on('close', (code) => {
            ...
            if (timedOut) {
                ...
                if (hadStreamingOutput) {
                    logger.info({ group: opts.groupName, containerName: opts.containerName, duration, code }, 'Container timed out after output (idle cleanup)');
                    settleOutputChain(() => {
                        resolve({ status: 'success', result: null, newSessionId });
                    });
                    return;
                }
                logger.error({ group: opts.groupName, containerName: opts.containerName, duration, code }, 'Container timed out with no output');
                const errorMsg = opts.timeoutErrorMessage ??
            ...
            if (isVerbose || isError) {
                logLines.push(`=== Input ===`, JSON.stringify(logInputObj, null, 2), ``);
            ...
            if (code !== 0) {
                ...
                if (signalInfo) {
                    ...
                    const stderrSnippet = maxChars > 0 ? stderr.slice(-maxChars) : stderr;
            ...
            if (opts.onOutput) {
                settleOutputChain(() => {
                    logger.info({ group: opts.groupName, duration, newSessionId }, 'Container completed (streaming mode)');
                    resolve({ status: 'success', result: null, newSessionId });
                });
                return;
            }
        ...
        container.on('error', (err) => {
            clearTimeout(timeout);
            logger.error({ group: opts.groupName, containerName: opts.containerName, error: err }, 'Container spawn error');
            resolve({
                status: 'error',
                result: null,
