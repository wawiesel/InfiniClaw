...
import { getActiveBots, bootstrapBot as serviceBootstrapBot, deployBot as serviceDeployBot, stopBot as serviceStopBot, rebuildImage as serviceRebuildImage, refreshStartScript as serviceRefreshStartScript, resolveRoot, instanceDir, loadProfileEnv, validateDeploy as serviceValidateDeploy, holodeckCreate as serviceHolodeckCreate, holodeckTeardown as serviceHolodeckTeardown, holodeckPromote as serviceHolodeckPromote, } from './service.js';
...
function botStatusLine(bot, emoji) {
    try {
...
function validateDeploy(bot) {
    return trySync(() => serviceValidateDeploy(resolveRoot(), bot), (err) => ({ ok: false, errors: errStr(err) }));
}
function deployInstance(bot) {
    return trySync(() => {
...
function rebuildImage(bot) {
    return trySync(() => { serviceRebuildImage(resolveRoot(), bot); return { ok: true, output: '' }; }, (err) => ({ ok: false, output: errStr(err) }));
}
function applyBrainMode(bot, mode, model) {
...
export function readBrainMode(bot) {
...
function requireMain(ctx, command) {
    if (ctx.isMain)
        return false;
...
async function safeSend(ctx, chatJid, text) {
    if (!chatJid)
        return;
    try {
        await ctx.sendMessage(chatJid, text);
    }
    catch (err) {
        logger.warn({ chatJid, err }, 'safeSend failed');
    }
}
/** Parse chatJid from data, returning null if empty. */
function parseChatJid(data) {
...
function parseBot(data) {
    if (typeof data.bot === 'string' && data.bot.trim()) {
        if (getActiveBots().includes(data.bot))
            return data.bot;
 ...
 */
export async function handleInfiniClawMessage(data, ctx) {
...
// ── Command handlers ────────────────────────────────────────────────────
async function handleSetBrainMode(data, ctx) {
    if (requireMain(ctx, 'set_brain_mode'))
        return;
    ...
    try {
        const summary = applyBrainMode(data.bot, data.mode, typeof data.model === 'string' ? data.model : undefined);
        logger.info({ bot: data.bot, mode: data.mode }, 'Brain mode updated via IPC');
        const chatJid = parseChatJid(data);
        if (chatJid)
            await ctx.sendMessage(chatJid, `${data.bot || ASSISTANT_NAME.toLowerCase()}:\n\n${summary}`);
    }
    catch (err) {
        logger.error({ err, data }, 'Failed to apply set_brain_mode');
    }
}
async function handleRestartBot(data, ctx) {
    if (requireMain(ctx, 'restart_bot'))
        return;
...
async function handleSelfRestart(bot, chatJid, ctx) {
...
async function handleCrossBotRestart(bot, chatJid, ctx) {
    ...
    try {
        ...
        if (fs.existsSync(groupsFile)) {
            ...
            if (mainGroup) {
                ...
                fs.writeFileSync(path.join(messagesDir, `restart-notice-${Date.now()}.json`), JSON.stringify(msg));
            }
        }
    }
    catch (err) {
...
async function handleStopBot(data, ctx) {
    if (requireMain(ctx, 'stop_bot'))
        return;
...
async function handleRebuildImage(data, ctx) {
    if (requireMain(ctx, 'rebuild_image'))
        return;
    const bot = parseBot(data);
    const chatJid = parseChatJid(data);
...
async function handleBotStatus(data, ctx) {
    if (requireMain(ctx, 'bot_status'))
        return;
    const bot = parseBot(data);
    const chatJid = parseChatJid(data);
    if (!chatJid)
...
function handleSetThread(data, ctx) {
...
async function handleRestartWksm(data, ctx) {
    if (requireMain(ctx, 'restart_wksm'))
        return;
    const chatJid = parseChatJid(data);
    ...
    try {
        ...
        if (chatJid)
            await ctx.sendMessage(chatJid, '🔄 Restarting wksm...');
        ...
        if (chatJid)
            await ctx.sendMessage(chatJid, `kill: ${killOut}`);
        ...
        if (chatJid)
            await ctx.sendMessage(chatJid, `✅ wksm started (pid ${child.pid}), health: ${health}`);
    }
    catch (err) {
        logger.error({ err }, 'restart_wksm failed');
        if (chatJid)
            await ctx.sendMessage(chatJid, `⛔ restart_wksm failed: ${err instanceof Error ? err.message : String(err)}`);
    }
}
async function handleGitPush(data, ctx) {
    if (requireMain(ctx, 'git_push'))
        return;
    const chatJid = parseChatJid(data);
...
// ── Holodeck handlers ────────────────────────────────────────────────────
async function handleHolodeckCreate(data, ctx) {
    if (requireMain(ctx, 'holodeck_create'))
        return;
    const bot = parseBot(data);
...
async function handleHolodeckTeardown(data, ctx) {
    if (requireMain(ctx, 'holodeck_teardown'))
        return;
    const bot = parseBot(data);
    const chatJid = parseChatJid(data);
...
async function handleHolodeckPromote(data, ctx) {
    if (requireMain(ctx, 'holodeck_promote'))
        return;
    const bot = parseBot(data);
    const chatJid = parseChatJid(data);
...
async function handleHolodeckSend(data, ctx) {
    if (requireMain(ctx, 'holodeck_send'))
        return;
    const bot = parseBot(data);
    ...
    try {
        ...
        try {
            ...
            const jidRow = db.prepare('SELECT jid FROM registered_groups LIMIT 1').get();
            const jid = jidRow?.jid || 'local:terminal';
            db.prepare('INSERT OR REPLACE INTO messages (id, chat_jid, sender, sender_name, content, timestamp, is_from_me) VALUES (?, ?, ?, ?, ?, ?, 0)').run(msgId, jid, 'operator', 'Captain', message, timestamp);
        }
        finally {
            db.close();
        }
...
async function handleHolodeckRead(data, ctx) {
    if (requireMain(ctx, 'holodeck_read'))
        return;
    const bot = parseBot(data);
    ...
    try {
        ...
        let rows;
        try {
            rows = db.prepare('SELECT sender_name, content, timestamp FROM messages ORDER BY timestamp DESC LIMIT ?').all(limit);
        }
        finally {
            db.close();
        }
        ...
        const formatted = rows.reverse().map((r) => `[${r.timestamp}] ${r.sender_name}: ${r.content.length > 200 ? r.content.slice(0, 200) + '...' : r.content}`).join('\n');
...
async function handleHolodeckStatus(data, ctx) {
    if (requireMain(ctx, 'holodeck_status'))
        return;
    const bot = parseBot(data);
    const chatJid = parseChatJid(data);
    if (!chatJid)
...
// ── send_to_room ────────────────────────────────────────────────────────
async function handleSendToRoom(data, ctx) {
...
function verificationsPath(root) {
    return path.join(root, '_runtime', 'data', 'verifications.json');
}
function readVerifications(root) {
...
function syncVerificationsToInstance(root, bot) {
    ...
    try {
        ...
        logger.warn({ bot, err }, 'Failed to sync verifications to instance');
    }
}
async function handleRequestVerification(data, ctx) {
    if (requireMain(ctx, 'request_verification'))
        return;
    const root = resolveRoot();
    const record = {
        id: data.id,
...
async function handleSubmitVerification(data, ctx) {
    if (requireMain(ctx, 'submit_verification'))
        return;
    const root = resolveRoot();
    const records = readVerifications(root);
    const record = records.find((r) => r.id === data.id);
    if (!record) {
        await safeSend(ctx, parseChatJid(data), `❌ Verification ${data.id} not found.`);
        return;
    }
    if (record.status !== 'pending') {
        await safeSend(ctx, parseChatJid(data), `Verification ${data.id} is already ${record.status}.`);
        return;
    }
...
export async function handleInfiniClawCommand(data, ctx) {
