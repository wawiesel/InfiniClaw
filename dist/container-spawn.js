...
import { buildBotDirectory, buildInfiniClawMounts, getPersonaContainerConfig, readPersonaGroupMcpServers, } from './container-mounts.js';
import { normalizeProviderSecrets, mapCertPathSecretsToContainer, } from './container-secrets.js';
import { recoverPodman, stopContainersByPrefix } from 'nanoclaw/podman-utils.js';
import { loadMachineConfig } from './machine-config.js';
import { CONTAINER_IMAGE, CONTAINER_MAX_OUTPUT_SIZE, CONTAINER_TIMEOUT, DATA_DIR, GROUPS_DIR, STORE_DIR, IDLE_TIMEOUT, ASSISTANT_NAME, } from 'nanoclaw/config.js';
...
function collectContainerSecrets(projectRoot) {
    const secrets = {};
...
function buildVolumeMounts(group, isMain, normalizedSecrets) {
    const mounts = [];
...
function buildContainerArgs(mounts, containerName, portPublish = [], memoryMb) {
    ...
    try {
        ...
        if (mc.containerNetwork)
            args.push('--network', mc.containerNetwork);
    }
...
function killExistingContainersForGroup(botTag, safeName) {
...
export async function runContainerAgent(group, input, onProcess, onOutput) {
    ...
    try {
        ...
        logger.warn({ err }, 'Failed to write bot directory to IPC');
    }
    ...
    try {
        if (loadMachineConfig().containerNetwork === 'host' && mcpServers) {
            const rewritten = JSON.parse(JSON.stringify(mcpServers).replace(/host\.containers\.internal/g, 'localhost'));
    ...
    const redactedInputForLog = {
        ...effectiveInput,
        secrets: redactSecrets(effectiveInput.secrets),
    };
    const mountLogLines = mounts.map((m) => `${m.hostPath} -> ${m.containerPath}${m.readonly ? ' (ro)' : ''}`);
    const mountSummaryLines = mounts.map((m) => `${m.containerPath}${m.readonly ? ' (ro)' : ''}`);
    return runContainer({
        runtime: 'podman',
        args: containerArgs,
...
//# sourceMappingURL=container-spawn.js.map