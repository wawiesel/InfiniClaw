...
import { resolveRoot, getActiveBots, bootstrapBot, stopBot, ensurePodmanReady, killStaleContainers, loadProfileEnv, updatePresence, } from './service.js';
// ── Config ─────────────────────────────────────────────────────────
...
function resolveCaptainUserId() {
    ...
    for (const bot of getActiveBots()) {
        try {
            ...
            if (env.CAPTAIN_USER_ID)
                return env.CAPTAIN_USER_ID;
        }
        catch { /* skip */ }
    }
    return '';
}
function isAuthorized(sender, captainUserId) {
    return sender === captainUserId || /-intercom:/.test(sender);
}
// ── Matrix API helpers ─────────────────────────────────────────────
async function matrixLogin(homeserver, username, password) {
...
async function matrixCreateFilter(homeserver, token, userId) {
    const filter = {
        room: {
    ...
    const resp = await fetch(`${homeserver}/_matrix/client/v3/user/${encodeURIComponent(userId)}/filter`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
        body: JSON.stringify(filter),
    });
    if (!resp.ok)
        throw new Error(`Filter creation failed: ${resp.status}`);
    const data = await resp.json();
    return data.filter_id;
}
async function matrixSync(homeserver, token, since, filterId, timeout) {
...
async function matrixSend(homeserver, token, roomId, text) {
    const txnId = `sv-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    const resp = await fetch(`${homeserver}/_matrix/client/v3/rooms/${encodeURIComponent(roomId)}/send/m.room.message/${encodeURIComponent(txnId)}`, {
        method: 'PUT',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
...
function buildBotRoomMap() {
    const root = resolveRoot();
    const map = {};
    for (const bot of getActiveBots()) {
        try {
            const env = loadProfileEnv(root, bot);
            if (env.MAIN_GROUP_NAME)
                map[bot] = env.MAIN_GROUP_NAME.toLowerCase();
        }
 ...
 */
function resolveBots(target, roomName) {
...
async function uploadHealthToS3(report) {
...
async function fetchAllHealthReports() {
    const s3 = getS3Client();
    if (!s3)
        return [];
    const results = [];
...
function formatHealthSummary(reports) {
    if (reports.length === 0)
        return '⚠️ No health reports available.';
    const lines = [`🏥 Fleet Health — ${new Date().toISOString().replace('T', ' ').slice(0, 19)} UTC\n`];
    let totalOom = 0;
    let totalSessions = 0;
...
function gitSync() {
    const root = resolveRoot();
    const execOpts = { cwd: root, encoding: 'utf-8', timeout: 30_000, stdio: 'pipe' };
...
async function gitSyncLoop(conns) {
                ...
                try {
                    ...
                    if (fs.existsSync(builtSupervisor)) {
                        for (const bot of getActiveBots()) {
                            ...
                            try {
                                fs.copyFileSync(builtSupervisor, dst);
                            }
...
const botDreamPhase = new Map();
const botDreamStartedAt = new Map();
const botIdleSince = new Map();
let dreamingBot = null; // only one at a time
/** Check if a bot has a running container. */
function hasRunningContainer(bot) {
    try {
...
function getContainerStartTime(bot) {
    try {
...
async function dreamLoop(conns) {
    ...
    while (true) {
        try {
            ...
            const botRooms = buildBotRoomMap();
            for (const bot of getActiveBots()) {
                if (botDreamPhase.get(bot) === 'dreaming' || botDreamPhase.get(bot) === 'recycling')
                    continue;
                ...
                if (isRunning)
                    botIdleSince.delete(bot);
                else if (!botIdleSince.has(bot))
                    botIdleSince.set(bot, now);
                ...
                if (!conn?.accessToken)
                    continue;
                const env = (() => { try {
                    return loadProfileEnv(root, bot);
                }
                catch {
                    return null;
                } })();
                const name = env?.ASSISTANT_NAME || bot;
                await matrixSend(conn.homeserver, conn.accessToken, conn.roomId, `${name}, begin your dream period. Review recent conversation history, consolidate your memory files, and optimize your standing orders for a fresh session. You have ${Math.floor(DREAM_DURATION_MS / 60_000)} minutes. Reply when done.`);
 ...
 */
async function heartbeatLoop(conns) {
    ...
    while (true) {
        try {
            ...
            for (const bot of getActiveBots()) {
                ...
                const env = loadProfileEnv(root, bot);
                const name = env?.ASSISTANT_NAME || bot;
                await matrixSend(conn.homeserver, conn.accessToken, conn.roomId, `${name}, check NEXT.md and work on the highest priority item you can act on.`);
...
// ── Command handling ───────────────────────────────────────────────
async function handleLifecycleCommand(action, target, conn) {
    const root = resolveRoot();
...
async function handleCommand(cmd, conn) {
    for (const action of ['join', 'dismiss', 'restart']) {
    ...
    if (cmd === '!roster') {
        ...
        if (roomBots.length === 0)
            return; // no local bots in this room
...
async function dialtone(conn, captainUserId) {
...
async function main() {
    ...
    const loops = conns.map((conn, i) => sleep(i * STARTUP_SYNC_DELAY).then(() => dialtone(conn, captainUserId)));
