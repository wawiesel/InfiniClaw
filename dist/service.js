...
function externalNanoclawDir(root) {
    return path.join(root, 'external', 'nanoclaw');
}
export function instanceDir(root, bot) {
...
function personaDir(root, bot) {
    return path.join(root, 'bots', resolveRole(bot), bot);
}
function profileEnvPath(_root, bot) {
    const config = loadMachineConfig();
    return path.join(config.secretsPath, bot, 'env');
}
/** Seed the registered_groups table with the main room from profile env. */
function seedMainRoomRegistration(instanceBase, mainJid, mainGroupName, mainGroupFolder, requiresTrigger) {
    ...
    seedDb.prepare(`INSERT OR REPLACE INTO registered_groups (jid, name, folder, trigger_pattern, added_at, requires_trigger)
     VALUES (?, ?, ?, ?, ?, ?)`).run(mainJid, mainGroupName, mainGroupFolder, '', new Date().toISOString(), requiresTrigger ? 1 : 0);
    seedDb.close();
}
// ── Env loading ────────────────────────────────────────────────────────
export function loadProfileEnv(root, bot) {
...
export function collectBotMatrixUserIds() {
    const config = loadMachineConfig();
    const ids = new Set();
    try {
        for (const bot of fs.readdirSync(config.secretsPath)) {
            ...
            if (env.MATRIX_USER_ID)
                ids.add(env.MATRIX_USER_ID);
        }
    }
    catch (err) {
        console.warn('Failed to read bot matrix user IDs:', err instanceof Error ? err.message : err);
    }
    return ids;
}
export function applyBrainEnv(env) {
    const out = { ...env };
    ...
    if (out.BRAIN_CA_CERT_FILE)
        out.NODE_EXTRA_CA_CERTS = out.BRAIN_CA_CERT_FILE;
...
function resolveOAuthToken() {
    try {
        const credJson = execSync('security find-generic-password -s "Claude Code-credentials" -w', { stdio: ['pipe', 'pipe', 'pipe'], encoding: 'utf-8' }).trim();
        if (!credJson)
            return '';
...
function checkTokenExpiry(expiresAt) {
    ...
    if (remaining <= 0) {
        console.error(`\x1b[31m[AUTH] OAuth token EXPIRED ${Math.abs(Math.round(days))} day(s) ago. ` +
            `Run \`claude setup-token\` to renew.\x1b[0m`);
    }
    else if (days <= TOKEN_EXPIRY_CRIT_DAYS) {
        console.error(`\x1b[31m[AUTH] OAuth token expires in ${Math.round(days * 24)} hours. ` +
            `Run \`claude setup-token\` to renew.\x1b[0m`);
    }
    else if (days <= TOKEN_EXPIRY_WARN_DAYS) {
        console.warn(`\x1b[33m[AUTH] OAuth token expires in ${Math.round(days)} day(s). ` +
            `Run \`claude setup-token\` to renew.\x1b[0m`);
    }
}
// ── Podman ─────────────────────────────────────────────────────────────
export function ensurePodmanReady() {
    ...
    if (!recoverPodman()) {
        throw new Error('Podman API unavailable after recovery attempt.\n' +
            'Try: podman machine stop podman-machine-default && podman machine start podman-machine-default');
    }
}
export function killStaleContainers(onlyBot) {
...
// ── Process cleanup ────────────────────────────────────────────────────
export function killRogueProcesses() {
    try {
 ...
 */
export function syncPersona(root, bot) {
...
export function updatePresence(root) {
    const config = loadMachineConfig();
...
export function writeCrewStatus(root, thisBot, dataDir) {
    const config = loadMachineConfig();
    const hostname = os.hostname();
    ...
    for (const [botId, info] of Object.entries(roster)) {
        const env = (() => { try {
            return loadProfileEnv(root, botId);
        }
        catch {
            return null;
        } })();
    ...
    for (const members of Object.values(crewByRoom)) {
        ...
        if (coBotId) {
            const env = (() => { try {
                return loadProfileEnv(root, coBotId);
            }
            catch {
                return null;
            } })();
            ...
            if (entry)
                entry.isCommandingOfficer = true;
        }
    }
...
export function restorePersona(root, bot) {
    const instance = instanceDir(root, bot);
    const persona = personaDir(root, bot);
    if (!fs.existsSync(persona))
        return;
 ...
 */
export function deployBot(root, bot) {
    const instance = instanceDir(root, bot);
    ...
    if (!fs.existsSync(path.join(instance, 'node_modules')) || !filesEqual(lockSrc, lockDst)) {
        ...
        try {
            fs.copyFileSync(path.join(instance, 'package-lock.json'), lockDst);
        }
        catch { /* ok */ }
    }
    // Build TypeScript
 ...
 */
export function validateDeploy(root, bot) {
    const instance = instanceDir(root, bot);
    ...
    if (fs.existsSync(instanceModules)) {
        ...
        try {
            fs.unlinkSync(stagingModules);
        }
        catch { /* ok */ }
        try {
            fs.rmSync(stagingModules, { recursive: true });
        }
        catch { /* ok */ }
...
export function rebuildImage(root, bot) {
...
function rebuildImageIfChanged(root, bot) {
 ...
 */
function rsyncInstance(root, dst, stdio = 'inherit') {
    ...
    if (fs.existsSync(baseClaude))
        fs.copyFileSync(baseClaude, path.join(dst, 'CLAUDE.md'));
}
/** Generate a shell wrapper that sources the env file at launch time. */
function generateStartScript(root, bot, nodeBin, instance) {
    const config = loadMachineConfig();
    const envFile = path.join(config.secretsPath, bot, 'env');
    return `#!/bin/bash
...
function pm2StartBot(bot, nodeBin, instance, logs, root) {
 ...
 */
export function refreshStartScript(root, bot) {
    const instance = instanceDir(root, bot);
    const startScript = path.join(instance, 'start.sh');
 ...
 */
export function bootstrapBot(root, bot) {
    const instance = instanceDir(root, bot);
...
export function startSupervisor(root) {
    const logs = logDir(root);
    fs.mkdirSync(logs, { recursive: true });
...
// ── Top-level commands ─────────────────────────────────────────────────
export async function start(onlyBot) {
    ...
    try {
        execSync(`"${PM2_BIN}" save`, { stdio: 'pipe' });
    }
    catch { /* ok */ }
    console.log('\nInfiniClaw running. Check status:\n  npx pm2 list');
}
export async function stop(onlyBot) {
    const root = resolveRoot();
    const allBots = getActiveBots();
    const bots = onlyBot ? allBots.filter(b => b === onlyBot) : allBots;
    for (const bot of bots) {
    ...
    try {
        ...
        const timeout = new Promise((_, reject) => setTimeout(() => reject(new Error('S3 push timed out after 30s')), 30_000));
...
export async function sync(direction) {
    const root = resolveRoot();
    if (direction === 'push') {
        await pushAll(root);
    }
...
export async function send(room, message) {
    const root = resolveRoot();
    const config = loadMachineConfig();
...
function holodeckBotName(bot) {
    return `${bot}-holodeck`;
}
export function holodeckCreate(bot, branch) {
...
export function holodeckChat(bot) {
    chat(holodeckBotName(bot));
}
export function holodeckTeardown(bot) {
    const root = resolveRoot();
    const hdBot = holodeckBotName(bot);
    const worktree = path.join(root, '_holodeck', bot);
    const instance = instanceDir(root, hdBot);
...
export function holodeckPromote(bot) {
    const root = resolveRoot();
    const worktree = path.join(root, '_holodeck', bot);
    if (!fs.existsSync(worktree)) {
    ...
    if (!branch)
        throw new Error('Cannot determine holodeck branch.');
