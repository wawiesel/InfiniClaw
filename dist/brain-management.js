...
function getConfiguredEnv(key) {
    return firstSet(process.env[key], PROJECT_ENV[key]);
}
function isMainConfiguredForOllama() {
    return isOllamaBaseUrl(getConfiguredEnv('ANTHROPIC_BASE_URL'));
}
export function resolveConfiguredMainModel() {
...
function bestModelByScore(entries, scorer) {
    let best;
    let bestScore = -1;
...
function getClaudeModelFromStatsCache() {
    ...
    if (modelUsage && typeof modelUsage === 'object') {
        const result = bestModelByScore(modelUsage, (usage) => {
    ...
    const dailyModelTokens = parsed.dailyModelTokens;
    if (Array.isArray(dailyModelTokens)) {
        for (let i = dailyModelTokens.length - 1; i >= 0; i -= 1) {
            ...
            if (!dayEntry || typeof dayEntry !== 'object')
                continue;
            const tokensByModel = dayEntry.tokensByModel;
            if (!tokensByModel || typeof tokensByModel !== 'object')
                continue;
            const result = bestModelByScore(tokensByModel, (v) => parseNumber(v));
            if (result)
                return result;
        }
    }
...
function isGenericClaudeModel(model) {
...
export function normalizeMainLlm(model) {
...
function applyOllamaFallbackToProfile(envFile) {
...
function isAnthropicQuotaError(text) {
...
export async function maybeAutoSwitchBrainsOnQuotaError(rawError, chatJid, sendMessage) {
    if (!isAnthropicQuotaError(rawError))
        return;
    if (Date.now() - lastAutoBrainSwitchAt < AUTO_BRAIN_SWITCH_COOLDOWN_MS)
        return;
    let config;
    try {
    ...
    for (const bot of config.bots) {
        ...
        if (!envPath.startsWith(config.secretsPath + path.sep))
    ...
    try {
        ...
        await sendMessage(chatJid, `Anthropic credits/quotas look exhausted. Switched ${switched.join(', ')} to ollama fallback. Restart bots to apply.`);
...
export function resolveMainLlm() {
...
export function setMainLlm(model) {
    mainLlm = model;
}
export function mainSender() {
...
export function defaultSenderForGroup(sourceGroup, registeredGroups) {
    if (sourceGroup === MAIN_GROUP_FOLDER) {
        return mainSender();
    }
    const groupName = Object.values(registeredGroups).find((g) => g.folder === sourceGroup)?.name;
    return groupName?.trim() || sourceGroup;
}
//# sourceMappingURL=brain-management.js.map