...
import { ASSISTANT_NAME, DATA_DIR, IDLE_TIMEOUT, POLL_INTERVAL, TRIGGER_PATTERN, } from 'nanoclaw/config.js';
import { ASSISTANT_ROLE, CAPTAIN_USER_ID, HEAP_LIMIT_MB, LOCAL_CHANNEL_ENABLED, LOCAL_MIRROR_MATRIX_JID, MAIN_GROUP_FOLDER, MATRIX_ACCESS_TOKEN, MATRIX_HOMESERVER, MATRIX_PASSWORD, MATRIX_RECONNECT_INTERVAL, MATRIX_USERNAME, MEMORY_CHECK_INTERVAL, RESUME_DELAY_SECONDS, validateConfig, } from './infini-config.js';
import { getAllChats, botParticipatesInThread, getMessagesSince, getNewMessages, getRecentMessages, getThreadMessages, getRouterState, initDatabase, deleteRegisteredGroup, deleteSession, setRegisteredGroup, setRouterState, setSession, storeChatMetadata, storeMessage, updateChatName, } from 'nanoclaw/db.js';
import { GroupQueue } from 'nanoclaw/group-queue.js';
import { writeGroupsSnapshot, } from 'nanoclaw/container-runner.js';
import { loadBaseState, saveBaseState, groupMessagesByChat, recoverPendingMessages, writeAgentSnapshots, wrapOnOutputForSession, } from 'nanoclaw/composables.js';
...
import { MAIN_PROVIDER, mainLlm, defaultSenderForGroup, resolveConfiguredMainModel, normalizeMainLlm, setMainLlm, maybeAutoSwitchBrainsOnQuotaError, } from './brain-management.js';
import { ensureChatActivity, getChatActivity, setObjectiveFromMessages, markRunStarted, markRunEnded, markProgress, markCompletion, markError, buildMainMissionContext, } from './chat-activity.js';
...
const lastProgressChatAt = {};
const PIP_PULSE = ['🔵', '🔷', '🔹', '🔷'];
const workThreadIds = {};
const activeReplyThreadIds = {};
// Per-turn thread for tool call <details> blocks when on main timeline
const progressToolCallThreadIds = {};
const threadMapLastSeen = {};
const triggerAckByMessageKey = {};
let resumeGateResolve = null;
const resumeGate = new Promise((resolve) => { resumeGateResolve = resolve; });
...
function handleLifecycleMessage(msg) {
    ...
    if (!msg.sender.includes('-intercom'))
        return false;
    const [, botName, action, rankStr] = match;
    const chatJid = msg.chat_jid;
    if (!roomRoster[chatJid])
        roomRoster[chatJid] = new Map();
    if (action === 'stopped') {
...
async function rerankCO(chatJid) {
    const roster = roomRoster[chatJid];
    if (!roster || roster.size === 0) {
        roomCO[chatJid] = undefined;
        return;
    }
    // Lowest rank = CO
    let coBotName;
...
function isCOMainTimelineTrigger(chatJid, messages) {
    if (roomCO[chatJid] !== ASSISTANT_NAME)
        return false;
    ...
    return messages.some(m => {
        if (botMatrixUserIds.has(m.sender))
            return false; // bot message
        if (m.thread_id)
            return false; // thread message, not main timeline
...
function resolveReplyThread(chatJid, messages) {
...
const kill137Consecutive = {};
const kill137CooldownUntil = {};
...
const standingOrderCycles = {};
const standingOrderRestUntil = {};
...
let botMatrixUserIds = new Set();
function isThreadContext(chatJid) {
    threadMapLastSeen[`r:${chatJid}`] = Date.now();
    return Boolean(activeReplyThreadIds[chatJid]);
}
function sendTriggerAck(chatJid, messages) {
    ...
    for (const m of messages) {
        if (!m.id)
            continue;
        if (/^(resume|out|system|op)-/.test(m.id))
            continue;
        const key = `${chatJid}:${m.id}`;
        if (triggerAckByMessageKey[key])
            continue;
...
function updateEventIdFile(groupFolder, key, eventId) {
    ...
    try {
        ...
        logger.debug({ groupFolder, key, err }, 'Failed to update event ID file');
    }
}
...
function createAdaptiveTimer(startedAt, onTick, registry, chatJid) {
...
function createIndicatorSet(emoji, activeVerb, doneVerb, delayMs) {
    const indicators = {};
    const delays = {};
    function start(chatJid, threadId) {
        if (indicators[chatJid] || delays[chatJid])
            return;
        ...
        const send = () => {
            if (indicators[chatJid])
                return;
            const ch = findChannel(channels, chatJid);
            if (!ch?.sendMessageReturningId || !ch?.editMessage)
                return;
    ...
    function clear(chatJid) {
        if (delays[chatJid]) {
            clearTimeout(delays[chatJid]);
            delete delays[chatJid];
            return;
        }
    ...
    function bump(chatJid, threadId) {
        if (delays[chatJid]) {
            clearTimeout(delays[chatJid]);
            delete delays[chatJid];
            start(chatJid, threadId);
            return;
        }
        const indicator = indicators[chatJid];
        if (!indicator) {
...
let outgoingSeq = 0;
function storeOutgoing(chatJid, text, threadId) {
...
function createOutputHandler(ctx) {
...
function handleProgressOutput(ctx, text) {
    ...
    if (isToolCall || !lastProgressChatAt[ctx.chatJid] || now - lastProgressChatAt[ctx.chatJid] >= PROGRESS_CHAT_COOLDOWN_MS) {
        if (!isToolCall)
            lastProgressChatAt[ctx.chatJid] = now;
        ...
        if (ch) {
            if (isToolCall) {
                ...
                if (activeThread) {
                    ...
                    const sendToToolThread = (threadId) => {
...
async function handleResultOutput(ctx, text) {
    clearIdleIndicator(ctx.chatJid);
    markProgress(ctx.chatJid, text);
    ...
    if (ch) {
        if (ch.setTyping)
            await ch.setTyping(ctx.chatJid, true);
        try {
            let sentEventId;
            threadMapLastSeen[`r:${ctx.chatJid}`] = Date.now();
            ...
            if (sentEventId) {
                ...
                if (group)
                    updateEventIdFile(group.folder, 'lastSent', sentEventId);
            }
        }
        finally {
            if (ch.setTyping)
                await ch.setTyping(ctx.chatJid, false);
        }
    }
}
...
async function processGroupMessages(chatJid) {
    ...
    if (killCooldownEnd > Date.now()) {
        ...
        logger.warn({ group: group.name, consecutiveKills: kill137Consecutive[chatJid], cooldownRemainingSec: remainSec }, 'Exit-137 cooldown active, deferring container spawn');
        return false;
    }
    ...
    const actionableMessages = filteredMessages.filter(m => {
        if (/^!operator\b/i.test(m.content.trim()))
            return false;
        if (!botMatrixUserIds.has(m.sender))
            return true; // human
    ...
    threadMapLastSeen[`r:${chatJid}`] = Date.now();
    logger.info({ group: group.name, replyThreadId: activeReplyThreadIds[chatJid], msgCount: contextMessages.length }, 'Thread routing resolved');
    const basePrompt = formatMessages(contextMessages);
    const threadContext = buildThreadContextBlock(chatJid, contextMessages);
    const missionContext = isMainGroup ? buildMainMissionContext(chatJid) : undefined;
    const parts = [];
    if (missionContext)
        parts.push(missionContext);
    if (threadContext)
    ...
    saveState();
    logger.info({ group: group.name, messageCount: contextMessages.length }, 'Processing messages');
    let idleTimer = null;
    ...
    if (channel?.setPresenceStatus)
        await channel.setPresenceStatus('online', 'processing...');
    if (!isResuming)
        startWorkingIndicator(chatJid, activeReplyThreadIds[chatJid]);
    const inboundMessageIds = contextMessages.map((m) => m.id).filter(Boolean);
    let hadError = false;
    ...
    let runProgressNudgeTimer = null;
    ...
    if (isMainGroup) {
        runProgressNudgeTimer = setInterval(() => {
            const now = Date.now();
            if (now - lastRunOutputAt < RUN_PROGRESS_NUDGE_STALE_MS)
                return;
            if (now - lastRunProgressNudgeAt < RUN_PROGRESS_NUDGE_COOLDOWN_MS)
                return;
            const activity = ensureChatActivity(chatJid);
            if (activity.lastCompletion && /\b(idle|done)\b/i.test(activity.lastCompletion))
                return;
            const nudged = queue.sendMessage(chatJid, 'If you are still running, send a concise progress update now: done, in-progress, next.');
            if (nudged) {
    ...
    if (channel?.setPresenceStatus)
        await channel.setPresenceStatus('online', 'idle');
    if (channel?.setStatusPip) {
    ...
    if (runResult.status === 'error' || hadError) {
        ...
        if (isSigKill) {
            ...
            if (kill137Consecutive[chatJid] >= KILL_137_MAX_CONSECUTIVE) {
                ...
                logger.warn({ group: group.name, consecutiveKills: kill137Consecutive[chatJid], cooldownMs: KILL_137_COOLDOWN_MS }, 'Repeated exit-137 kills — applying respawn cooldown');
        ...
        if (!outputSentToUser && channel) {
            ...
            try {
                ...
                logger.warn({ group: group.name, err: sendErr }, 'Failed to send error reply to channel');
            }
        }
        if (outputSentToUser) {
    ...
    markRunEnded(chatJid);
    if (isMainGroup)
        scheduleStandingOrderCycle(chatJid);
    appendConversationLog(group.folder, missedMessages, agentResponses, channel?.name);
    return true;
}
// ── Run agent ──────────────────────────────────────────────────────────
 ...
 * interrupt message.
 */
function sendInterruptMessage(chatJid, group, text) {
...
async function handleGroupMessagesInLoop(chatJid, groupMessages) {
    const group = registeredGroups[chatJid];
    if (!group)
        return;
    const isMainGroup = group.folder === MAIN_GROUP_FOLDER;
    ...
    const allPending = getMessagesSince(chatJid, lastAgentTimestamp[chatJid] || '', ASSISTANT_NAME).filter((msg) => {
    ...
    const groupStatus = queue.getGroupStatus(chatJid);
    if (groupStatus.active) {
        const interruptMessages = messagesToSend.filter((m) => TRIGGER_PATTERN.test(m.content.trim()) || (CAPTAIN_USER_ID && m.sender === CAPTAIN_USER_ID));
        if (interruptMessages.length > 0) {
            startWorkingIndicator(chatJid, activeReplyThreadIds[chatJid]);
            const ch = findChannel(channels, chatJid);
            if (ch?.setTyping)
                ch.setTyping(chatJid, true).catch((err) => { logger.debug({ chatJid, err }, 'Set typing failed'); });
...
async function handlePipedToActiveContainer(chatJid, messagesToSend) {
    ...
    if (ch?.setPresenceStatus)
        await ch.setPresenceStatus('online', 'processing...');
}
function handleQueuedForProcessing(chatJid) {
...
function scheduleStandingOrderCycle(chatJid) {
    if ((standingOrderRestUntil[chatJid] || 0) > Date.now())
        return;
    const activity = ensureChatActivity(chatJid);
    ...
    setTimeout(() => {
        ...
        if (state.active || state.pendingMessages || state.pendingTasks > 0)
            return;
...
function buildThreadContextBlock(chatJid, messages) {
...
function injectSystemNotice(chatJid, content) {
    storeMessage({
...
// ── Recovery & resume ──────────────────────────────────────────────────
async function injectResumeMessage() {
    isResuming = true;
...
async function main() {
    ...
    const shutdown = async (signal) => {
        ...
        for (const timer of persistentTimers)
            clearInterval(timer);
        for (const jid of Object.keys(registeredGroups)) {
            const ch = findChannel(channels, jid);
            if (ch?.setStatusPip) {
                try {
                    await ch.setStatusPip(jid, '🔴');
                }
                catch { /* best-effort */ }
            }
        }
        for (const ch of channels) {
            if (ch.setPresenceStatus)
                await ch.setPresenceStatus('offline', 'shutting down...');
        }
        if (matrixRef) {
            try {
                await matrixRef.setDisplayName(`${ASSISTANT_NAME} 🔴`);
    ...
    if (MATRIX_HOMESERVER &&
        ...
        matrix = new MatrixChannel({
            ...
            onMessage: (_chatJid, msg) => {
                if (handleOperatorCommand(msg, matrix, injectSystemNotice))
                    return;
                ...
                if (msg.id && msg.id.startsWith('$')) {
                    ...
                    if (group)
                        updateEventIdFile(group.folder, 'lastReceived', msg.id);
                }
            },
    ...
    const refreshConnectedChannels = () => {
        channels = allChannels.filter((ch) => ch != null && ch.isConnected());
    };
    if (localCli) {
        try {
    ...
    persistentTimers.push(setInterval(() => {
        ...
        if (count > 0)
            logger.info({ count }, 'Pruned expired allow-list entries');
    }, 5 * 60 * 1000));
    ...
    persistentTimers.push(setInterval(() => {
        ...
        for (const jid of Object.keys(workThreadIds)) {
            if (!threadMapLastSeen[`w:${jid}`])
                threadMapLastSeen[`w:${jid}`] = now;
        ...
        for (const jid of Object.keys(activeReplyThreadIds)) {
            if (!threadMapLastSeen[`r:${jid}`])
                threadMapLastSeen[`r:${jid}`] = now;
        ...
        for (const jid of Object.keys(progressToolCallThreadIds)) {
            if (!threadMapLastSeen[`p:${jid}`])
                threadMapLastSeen[`p:${jid}`] = now;
        ...
        if (pruned > 0)
            logger.info({ pruned }, 'Pruned stale thread map entries');
    }, STALE_THREAD_CHECK));
    ...
    const writeStatusSnapshot = () => {
        try {
            const snapshot = {
                ...
                brainModes: Object.fromEntries(getActiveBots().map((b) => [b, readBrainMode(b)])),
                groups: Object.entries(registeredGroups).map(([jid, g]) => {
                    const queueStatus = queue.getGroupStatus(jid);
                    const activity = getChatActivity(jid) || {};
                    return {
            ...
            try {
                if (fs.existsSync(vPath)) {
                    const vRecords = JSON.parse(fs.readFileSync(vPath, 'utf-8'));
                    ...
                    snapshot.verifications = { pending, verified, failed, total: vRecords.length };
                }
            }
            catch { /* ok */ }
    ...
    startIpcWatcher({
        ...
        defaultSenderForGroup: (sourceGroup) => defaultSenderForGroup(sourceGroup, registeredGroups),
        sendImage: (jid, buffer, filename, mimetype, caption) => {
            const ch = findChannel(channels, jid);
            if (ch?.sendImage)
                return ch.sendImage(jid, buffer, filename, mimetype, caption);
        ...
        sendFile: (jid, buffer, filename, mimetype, caption) => {
            const ch = findChannel(channels, jid);
            if (ch?.sendFile)
                return ch.sendFile(jid, buffer, filename, mimetype, caption);
        ...
        registerGroup,
        unregisterGroup,
        setWorkThread: (chatJid, threadId) => {
            if (threadId) {
            ...
            if (group) {
                ...
                try {
                    ...
                    logger.debug({ chatJid, err }, 'Failed to write workThread to IPC file');
                }
            }
        },
        syncGroups: async () => { },
    ...
    persistentTimers.push(setInterval(() => {
        for (const [chatJid, group] of Object.entries(registeredGroups)) {
            ...
            if (!status.active || status.idleWaiting)
                continue;
            queue.sendMessage(chatJid, '[System] Periodic checkpoint: if you have completed or are mid-way through any tasks, save a brief summary to your memory now using /save-memory. Include what you were doing and what remains.');
    ...
    function buildStartupChecklist() {
        ...
        try {
            ...
            skillLines = '_(error reading skills)_';
        }
        ...
        try {
            ...
            mcpLines = '_(error reading MCP)_';
        }
        ...
        let todoLines = '';
        if (activeTodos.length > 0) {
            const statusEmoji = { in_progress: '🔄', pending: '⏳' };
        ...
        if (isEngineer) {
            ...
            try {
                ...
                healthLines = '_(error reading health)_';
            }
        ...
        if (isNavigator) {
            ...
            try {
                ...
                goalLines = '_(error reading goals)_';
            }
            ...
            try {
                ...
                knowledgeEntry = '_(error reading knowledge)_';
            }
    ...
    const bootAnnounceTimer = setInterval(async () => {
        ...
        try {
            if (ch.setPresenceStatus)
                await ch.setPresenceStatus('online', 'idle');
