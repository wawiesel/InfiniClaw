 ...
 */
export function normalizeProviderSecrets(secrets) {
 ...
 */
function normalizeCertBundleEnv(mapped) {
    const certBundle = mapped.SSL_CERT_FILE ||
    ...
    for (const key of CERT_PATH_ENV_VARS) {
        if (!mapped[key])
            mapped[key] = certBundle;
    }
}
/**
 * Map a single cert path to container, adding mount if needed.
 */
function mapCertPath(key, value, mounts, certMountRoot) {
    ...
    const alreadyMounted = mounts.some((m) => m.hostPath === value && m.containerPath === containerPath);
    if (!alreadyMounted) {
 ...
 */
export function mapCertPathSecretsToContainer(secrets, mounts) {
...
//# sourceMappingURL=container-secrets.js.map