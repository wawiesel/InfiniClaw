...
const server = http.createServer((req, res) => {
  ...
  // Token usage dashboard — tokenDashboardPage defined below server block
  if (url === `${BASE}/tokens`) {
    const testMode = (req.url || '').includes('test=1');
    res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'no-cache' });
  ...
  if (url === `${BASE}/tokens/data.json`) {
    void (async () => {
      try {
        ...
        const { loadFleet } = await import('./ship-config.js');
        ...
        for (const bot of bots) {
          ...
          if (entries.length > 0 || Object.keys(agg).length > 0) {
            result[bot] = { entries, aggregate: agg };
          }
        }
        res.writeHead(200, { 'Content-Type': 'application/json', 'Cache-Control': 'no-cache' });
...
<script>
const MODEL_COLORS = {'claude-sonnet-4-6':'#58a6ff','claude-opus-4-6':'#bc8cff','qwen3:14b':'#3fb950','qwen3:30b':'#2ea043'};
...
function testData() {
  ...
  for (let i=0;i<120;i++) {
    ...
    entries.push({timestamp:new Date(now-h*3600000).toISOString(),model:M[mi].m,provider:M[mi].p,
      input_tokens:Math.floor(b*.6),output_tokens:Math.floor(b*.3),cache_tokens:Math.floor(b*.1)});
  }
  return {parker:{entries,aggregate:{'anthropic/claude-sonnet-4-6':{input:28000,output:14000,cache:4000,total:46000},
    'ollama/qwen3:14b':{input:12000,output:6000,cache:0,total:18000},'anthropic/claude-opus-4-6':{input:20000,output:10000,cache:5000,total:35000}}}};
}
...
function computeRate(entries) {
  ...
  const pts = sorted.map(e => {
    const t = (e.input_tokens||0)+(e.output_tokens||0)+(e.cache_tokens||0);
    cum += (e.tokens||t);
    return { ts: new Date(e.timestamp).getTime(), cum, model: e.model||'unknown' };
  });
  // Central difference derivative → tokens/hr
  const rate = [];
  for (let i=0; i<pts.length; i++) {
    const prev = pts[Math.max(0,i-1)], next = pts[Math.min(pts.length-1,i+1)];
    const dt = (next.ts - prev.ts) / 3600000; // hours
    const dy = next.cum - prev.cum;
    rate.push({ ts: new Date(pts[i].ts), rate: dt>0 ? dy/dt : 0, model: pts[i].model });
  }
  return rate;
}
...
async function load() {
  ...
  for (const bot of bots) {
    ...
    if (d.aggregate && Object.keys(d.aggregate).length) {
      html += '<h2>'+bot+' — 7d totals</h2><table><tr><th>Model</th><th>Input</th><th>Output</th><th>Cache</th><th>Total</th></tr>';
      for (const [m,v] of Object.entries(d.aggregate))
        html += '<tr><td>'+m+'</td><td>'+v.input.toLocaleString()+'</td><td>'+v.output.toLocaleString()+'</td><td>'+v.cache.toLocaleString()+'</td><td>'+v.total.toLocaleString()+'</td></tr>';
      html += '</table>';
    }
    html += '<div id="plot-'+bot+'"></div>';
  }
  // Legend
  html += '<div style="margin:12px 0;display:flex;gap:16px;flex-wrap:wrap">';
  for (const [m,c] of Object.entries(MODEL_COLORS))
  ...
  for (const bot of bots) {
    ...
    if (!rate.length) continue;

    // Build connected segments colored by model
    const traces = [];
    let segStart = 0;
    for (let i=1; i<=rate.length; i++) {
      if (i===rate.length || rate[i].model !== rate[segStart].model) {
        const seg = rate.slice(segStart, Math.min(i+1, rate.length)); // overlap 1 point
        traces.push({
          x: seg.map(p=>p.ts), y: seg.map(p=>Math.round(p.rate)),
          type:'scatter', mode:'lines', line:{color:MODEL_COLORS[rate[segStart].model]||DEFAULT_COLOR, width:2.5, shape:'spline'},
          name:rate[segStart].model, showlegend:false,
          hovertemplate:rate[segStart].model+': %{y:,.0f} tok/hr<extra></extra>'
        });
        segStart = i;
      }
    }

    Plotly.newPlot('plot-'+bot, traces, {
      title:{text:bot+' — token burn rate',font:{color:'#e6edf3',size:14}},
      paper_bgcolor:'#161b22', plot_bgcolor:'#0d1117',
      font:{color:'#8b949e',size:11},
      xaxis:{gridcolor:'#21262d',linecolor:'#30363d',autorange:'reversed',title:{text:'\u2190 past \u00b7 now \u2192'},type:'date'},
      yaxis:{gridcolor:'#21262d',linecolor:'#30363d',title:{text:'tokens/hr'},rangemode:'tozero'},
      height:250, margin:{t:40,b:50,l:70,r:20}, hovermode:'x unified', showlegend:false
    }, {responsive:true,displayModeBar:true,scrollZoom:true,modeBarButtonsToRemove:['lasso2d','select2d']});
  }
}
load();
</script>`;
