...
export function handleOperatorCommand(
    ...
    if (cmd === '!') {
        const help = [
            ...
            '  !promote <target>         — raise rank (bot or ship)',
            '  !demote <target>          — lower rank (bot or ship)',
            '  !commission [ship]        — commission ship, start bots',
            '  !decommission [ship]      — stop all bots, keep helm',
            '  !provision [target]       — sync repos',
            '  !refit [ship]             — full overhaul: sync, rebuild, restart',
            '  !allow <bot> <path> [min] — grant rw mount (authorized)',
            '  !deny <bot> <path>        — revoke rw mount (authorized)',
            '  !helm <text>              — send to operator tmux',
