...
├── command-registry.ts → Single source of truth for x-command names (includes context-aware !metrics). Help text for !pull says "restart bots" (preserves state, not "wake"). `buildHelpText()` wraps output in a markdown code block for fixed-width font rendering.
