...
├── metrics.ts          → Fleet metrics: operator interventions (non-command only), bot scores, ship uptime + infra failure rate, fleet availability/autonomy, response latency (p50/p95), token throughput (rolling 1d/7d from session JSONL via `readTokenThroughput`, cached 60s in `tokenThroughputCache`), MTBI. Publishes to S3. `recordMessageDelivery()`/`recordBotReply()` track response latency. MTBI computed from intervention gaps in `OperatorMetrics`. All metrics show `?` placeholder when no data, so metric spots remain visible. Build failures during `!wake` counted via `recordInfraFailure('wake-build')`.
...
- **Lifecycle message format**: `sendLifecycleMsg()` broadcasts `<shipEmoji>HOSTNAME: BotName stopped|started|reranked (rank N)` via the bot's duty room intercom. The ship emoji prefix (from `findShipByHostname()?.[1]?.emoji`) identifies the originating ship. `handleLifecycleMessage()` in `main.ts` matches via `^\S+:` so the attached emoji is transparent to parsing.
