...
├── service.ts          → Deploy, start, stop bots via pm2; seeds quarters or duty room based on fleet status; restartBotForRoom for lightweight room switches. `computeBuildContextHash` hashes `bots/container/` (the actual build context from `bots/build.sh`) + bot Dockerfile to detect when image rebuild is needed (fixes #44).
