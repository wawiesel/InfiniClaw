...
- **Containerized branch brains**: `spawnBranchBrain()` spawns via `podman run` when `BRANCH_BRAIN_IMAGE` is set (default: `localhost/infiniclaw-branch-brain:latest`). Container runs with `--network slirp4netns` (rootless NAT — allows API access without exposing host ports), memory/pid limits, and credentials passed as `--env` args rather than inherited host env. The thread-notes dir is bind-mounted at `/relay-notes` so findings persist to the host. Set `BRANCH_BRAIN_IMAGE=` (empty) to fall back to host `claude` spawn. Dockerfile at `bots/container/branch-brain/Dockerfile` (issue #19).
