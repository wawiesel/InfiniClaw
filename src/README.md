...
- **Bot tree lines**: Use `botTreeLine(isLast, badge, nameDisplay, role, roleIcon, rank, isChief, rolePad, suffix)` from `formatting.ts`. Consistent `├`/`└` prefix, badge, name, role, rank medal layout. Used by `!metrics`. `!fleet` uses `unifiedBotDisplay(params, 'medium')` instead — compact emoji+abbreviated-text format (e.g. `🦁Herc⚙️Eng·🟢A·Cid·⚙️eng🥈1`).
