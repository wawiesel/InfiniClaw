...
├── health-check.ts     → Health data collection: reads bot logs, parses sigkills/sigterms/OOM/spawns/memory, falls back to metrics-history.jsonl. `sessionCleanup()` prunes telemetry/, debug/, and old JSONL files per-bot. Token throughput computed from session JSONL files. `SyncStatus` and `HealthReport` interfaces include optional beacon fields (`relay_uptime_s`, `secrets_sync`, `git_sync`) populated by relay before S3 upload.
...
- **Resume context size**: `injectResumeMessage()` fetches the last **5** recent messages (not 10) to keep resume prompts small. Larger resume context can cause the first Claude turn to exceed the 90s brain timeout, triggering a restart loop (issue #21).
