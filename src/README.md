...
- **Room-scoped commands**: `resolveBots(target, conn, presenceOnly)` — when `presenceOnly=true`, matches bots physically IN the room (onduty in duty rooms, quarters/sleep in quarters rooms). `!report` uses assignment mode (finds bots to bring in). BTC always uses assignment mode (universal command room). Explicit targets not present get `relay Name not in this room` warning.
