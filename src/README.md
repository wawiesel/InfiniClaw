...
├── service.ts          → Deploy, start, stop bots via launchd
...
├── ipc-commands.ts     → Handle restart_bot, rebuild_image, git_push, etc.
...
├── operator-commands.ts→ !allow, !deny, !todo, system commands
...
**Start:** `cli.ts` → `service.ts:startAll()` → for each bot in `machine.json`: `deployBot()` → rsync nanoclaw, write crew status, install launchd plist.
