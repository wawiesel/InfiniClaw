...
describe('restoreMentionPrefixes', () => {
  ...
  it('wraps case-insensitive @name from pill', () => {
    ...
    expect(restoreMentionPrefixes(body, html)).toBe('{{mention Cid}} do something');
  });
  ...
  it('handles mention at end of message', () => {
    ...
    expect(restoreMentionPrefixes(body, html)).toBe('hey {{mention Cid}}');
  });
  ...
  it('handles display names with special regex characters', () => {
    ...
    expect(restoreMentionPrefixes(body, html)).toBe('{{mention C.I.D}} hello');
  });
  ...
  it('does not wrap partial name matches', () => {
    ...
    expect(result).toContain('{{mention Cid}}');
    expect(result).not.toContain('{{mention Cider}}');
  });
...
// --- pillifyMentions (outbound: {{mention Name}} → pill) ---
...
describe('pillifyMentions', () => {
  ...
  it('converts {{mention Name}} to a mention pill', () => {
    const result = pillifyMentions('{{mention Cid}} hello', cache);
  ...
  it('is case-insensitive on name match', () => {
    const result = pillifyMentions('{{mention cid}} status', cache);
    expect(result).toContain('href="https://matrix.to/#/@cid:a-gis.org"');
  });

  it('converts multiple markers in same message', () => {
    const result = pillifyMentions('{{mention Cid}} and {{mention Nora}}', cache);
  ...
  it('strips marker for unknown names, leaving plain text', () => {
    const result = pillifyMentions('{{mention Unknown}} hello', cache);
  ...
  it('returns text with stripped markers when cache is empty', () => {
    const result = pillifyMentions('{{mention Cid}} hello', new Map());
    expect(result).toBe('Cid hello');
  });

  it('matches display names with pips by base name only', () => {
    const result = pillifyMentions('{{mention Cid}} reporting', cache);
    expect(result).toContain('href="https://matrix.to/#/@cid:a-gis.org"');
    expect(result).toContain('>Cid</a>');
  });

  it('handles {{mention operator}} mention', () => {
    const result = pillifyMentions('{{mention operator}} help', cache);
  ...
  it('works inside HTML tags', () => {
    const html = '<p><strong>{{mention Cid}}</strong> can you check?</p>';
  ...
  it('handles marker in pre-formatted HTML', () => {
    const html = '<font color="#888">Status: {{mention Cid}} is online</font>';
    const result = pillifyMentions(html, cache);
    expect(result).toContain('href="https://matrix.to/#/@cid:a-gis.org"');
  });
  ...
  it('escapes special characters in userId for HTML safety', () => {
    ...
    const result = pillifyMentions('{{mention Evil}} hi', evilCache);
...
// --- convertRawMentions (inbound: @Name → {{mention Name}}) ---
...
describe('convertRawMentions', () => {
  ...
  it('converts @Name to {{mention Name}}', () => {
    expect(convertRawMentions('@Cid hello', cache)).toBe('{{mention Cid}} hello');
  });
  ...
  it('converts multiple @Names', () => {
    ...
    expect(result).toBe('{{mention Cid}} and {{mention Nora}} review');
  });
  ...
  it('converts @Name at end of message', () => {
    expect(convertRawMentions('hey @Cid', cache)).toBe('hey {{mention Cid}}');
  });

  it('converts @Name with punctuation after', () => {
    expect(convertRawMentions('@Cid, can you help?', cache)).toBe('{{mention Cid}}, can you help?');
  });
