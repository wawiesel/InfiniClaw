...
| `formatting.test.ts` | rankMedal (chief/rank1/rank2/rank3+), botBadge (sleep/transit/health/grade/activity), botTreeLine (medals, prefix), unifiedBotDisplay (short/long × all fields, optional nameWidth padding for column alignment, long uses emoji[value] without word labels, health grade shown for all statuses including sleep, activity always shown including ⏸ for idle, M/K tok formatting), unifiedShipDisplay (short/long, speaker/non-speaker/decommissioned, idle shows ⏸). |
