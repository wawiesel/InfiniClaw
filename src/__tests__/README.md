...
| `formatting.test.ts` | rankMedal (chief/rank1/rank2/rank3+), botBadge (sleep/transit/health/grade/activity), botTreeLine (medals, prefix), unifiedBotDisplay (short/long × all fields, long uses emoji[value] without word labels, health grade shown for all statuses including sleep, activity shown for all statuses, M/K tok formatting), unifiedShipDisplay (short/long, speaker/non-speaker/decommissioned). |
