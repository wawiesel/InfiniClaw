...
| `metrics.test.ts` | Fleet metrics: rollingRate, SCORE_REACTIONS, operator/bot/ship/fleet metrics, autonomy score (x-commands excluded), infra failure tracking, branch brain success, scope routing (bot-scoped returns empty on non-owning ships), formatting with fleet-aligned badges ◉/❓ (46 tests). `relayRestarts` is `RollingMetric` (1d/7d), not cumulative. |
