...
| `formatting.test.ts` | rankMedal (chief/rank1/rank2/rank3+), botBadge (sleep/transit/health/grade/activity), unifiedBotDisplay (short format space-separates icons: `⚙️ 🥇 🟢 🔥`, long uses emoji[value] without word labels, optional nameWidth padding, health grade shown for all statuses including sleep, activity always shown including ⏸ for idle, M/K tok formatting), unifiedShipDisplay (short/long with spaced icons, speaker/non-speaker/decommissioned, idle shows ⏸). |
