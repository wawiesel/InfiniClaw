...
| `matrix-mentions.test.ts` | Mention pill symmetry: inbound `<m>` wrapping, outbound `<m>` → pill conversion, raw `@Name` → `<m>` conversion (display names use realistic pip-prefix format) |
