...
| `metrics.test.ts` | Fleet metrics: rollingRate, SCORE_REACTIONS, operator/bot/ship/fleet metrics, autonomy score (x-commands excluded), infra failure tracking, branch brain success, response latency tracking, MTBI in operator metrics, scope routing, formatting per `docs/design/20-metrics.md` — badges ◉ (running), 🔴 (down), 💤 (sleep). |
